...
# Due to how error is handled in input 1 and input 2, with a while loop to handle exceptions, we have yet to find ways to perform pytest for the raised exceptions

# def test_InvalidOperation1(monkeypatch):
#     with pytest.raises(InvalidOperation):
#         input1()
#         monkeypatch.setattr('builtins.input', lambda _: "notanumber")

# def test_LessThanZeroException1(monkeypatch):
#     with pytest.raises(LessThanZeroException):
#         input1()
...
def test_0(monkeypatch):
    ...
    next_calc, mpf1, mpf2, jtax, btax = main(Decimal("0"), Decimal("0"))

    # replace assertions by conditions
    if not mpf1 == Decimal("0"):
        errors.append("Person 1 MPF != 0")
    if not mpf2 == Decimal("0"):
        errors.append("Person 2 MPF != 0")
    if not btax == Decimal("0"):
        errors.append("Combined Tax Payable != 0")
    if not jtax == Decimal("0"):
        errors.append("Joint Tax Payable != 0")

    # assert no error message has been registered, else print messages
    assert not errors, "Errors occured:\n{}".format("\n".join(errors))


def test_nompf(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("85199.99"), Decimal("85199.99"))
...
def test_basempf(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("85200"), Decimal("85200"))

    # replace assertions by conditions
    if not mpf1 == Decimal("4260"):
        errors.append("Person 1 MPF != 4260")
    if not mpf2 == Decimal("4260"):
        errors.append("Person 2 MPF != 4260")
    if not btax == Decimal("0"):
        errors.append("Combined Tax Payable != 0")
    if not jtax == Decimal("0"):
        errors.append("Joint Tax Payable != 0")
...
def test_maxmpf(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("360000"), Decimal("360000"))

    # replace assertions by conditions
    if not mpf1 == Decimal("18000"):
        errors.append("Person 1 MPF != 18000")
    if not mpf2 == Decimal("18000"):
        errors.append("Person 2 MPF != 18000")
    if not btax == Decimal("35400"):
        errors.append("Combined Tax Payable != 35400")
    if not jtax == Decimal("53400"):
        errors.append("Joint Tax Payable != 53400")

    # assert no error message has been registered, else print messages
    assert not errors, "Errors occured:\n{}".format("\n".join(errors))


def test_maxnotax(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("138947.36"), Decimal("138947.36"))

    # replace assertions by conditions
    if not round(mpf1, 2) == Decimal("6947.37"):
        errors.append("Person 1 MPF != 6947.37")
    if not round(mpf2, 2) == Decimal("6947.37"):
        errors.append("Person 2 MPF != 6947.37")
    if not round(btax, 2) == Decimal("0"):
        errors.append("Combined Tax Payable != 0")
    if not round(jtax, 2) == Decimal("0"):
        errors.append("Joint Tax Payable != 0")
...
def test_taxbracket1(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("165263.15"), Decimal("165263.15"))

    # replace assertions by conditions
    if not round(mpf1, 2) == Decimal("8263.16"):
        errors.append("Person 1 MPF != 8263.16")
    if not round(mpf2, 2) == Decimal("8263.16"):
        errors.append("Person 2 MPF != 8263.16")
    if not round(btax, 2) == Decimal("1000"):
        errors.append("Combined Tax Payable != 1000")
    if not round(jtax, 2) == Decimal("1000"):
        errors.append("Joint Tax Payable != 1000")

    # assert no error message has been registered, else print messages
    assert not errors, "Errors occured:\n{}".format("\n".join(errors))


def test_taxbracket2(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("191578.94"), Decimal("191578.94"))

    # replace assertions by conditions
    if not round(mpf1, 2) == Decimal("9578.95"):
        errors.append("Person 1 MPF != 9578.95")
    if not round(mpf2, 2) == Decimal("9578.95"):
        errors.append("Person 2 MPF != 9578.95")
    if not round(btax, 2) == Decimal("2000"):
        errors.append("Combined Tax Payable != 2000")
    if not round(jtax, 2) == Decimal("4000"):
        errors.append("Joint Tax Payable != 4000")

    # assert no error message has been registered, else print messages
    assert not errors, "Errors occured:\n{}".format("\n".join(errors))


def test_taxbracket3(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("217894.73"), Decimal("217894.73"))

    # replace assertions by conditions
    if not round(mpf1, 2) == Decimal("10894.74"):
        errors.append("Person 1 MPF != 10894.74")
    if not round(mpf2, 2) == Decimal("10894.74"):
        errors.append("Person 2 MPF != 10894.74")
    if not round(btax, 2) == Decimal("5000"):
        errors.append("Combined Tax Payable != 5000")
    if not round(jtax, 2) == Decimal("9000"):
        errors.append("Joint Tax Payable != 9000")

    # assert no error message has been registered, else print messages
    assert not errors, "Errors occured:\n{}".format("\n".join(errors))


def test_taxbracket4(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("244210.52"), Decimal("244210.52"))

    # replace assertions by conditions
    if not round(mpf1, 2) == Decimal("12210.53"):
        errors.append("Person 1 MPF != 12210.53")
    if not round(mpf2, 2) == Decimal("12210.53"):
        errors.append("Person 2 MPF != 12210.53")
    if not round(btax, 2) == Decimal("8000"):
        errors.append("Combined Tax Payable != 8000")
    if not round(jtax, 2) == Decimal("16000"):
        errors.append("Joint Tax Payable != 16000")

    # assert no error message has been registered, else print messages
    assert not errors, "Errors occured:\n{}".format("\n".join(errors))


def test_chan(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("540000"), Decimal("120000"))

    # replace assertions by conditions
    if not round(mpf1, 2) == Decimal("18000"):
        errors.append("Person 1 MPF != 18000")
    if not round(mpf2, 2) == Decimal("6000"):
        errors.append("Person 2 MPF != 6000")
    if not round(btax, 2) == Decimal("48300"):
        errors.append("Combined Tax Payable != 48300")
    if not round(jtax, 2) == Decimal("45240"):
        errors.append("Joint Tax Payable != 45240")

    # assert no error message has been registered, else print messages
    assert not errors, "Errors occured:\n{}".format("\n".join(errors))


def test_chan2(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("540000"), Decimal("420000"))

    # replace assertions by conditions
    if not round(mpf1, 2) == Decimal("18000"):
        errors.append("Person 1 MPF != 18000")
    if not round(mpf2, 2) == Decimal("18000"):
        errors.append("Person 2 MPF != 18000")
    if not round(btax, 2) == Decimal("76200"):
        errors.append("Combined Tax Payable != 76200")
    if not round(jtax, 2) == Decimal("94200"):
        errors.append("Joint Tax Payable != 94200")

    # assert no error message has been registered, else print messages
    assert not errors, "Errors occured:\n{}".format("\n".join(errors))


def test_stdtax1(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("3500000"), Decimal("5000000"))
    ...
    if not round(mpf2, 2) == Decimal("18000"):
        errors.append("Person 2 MPF != 18000")
    if not round(btax, 2) == Decimal("1269600"):
        errors.append("Combined Tax Payable != 1269600")
    if not round(jtax, 2) == Decimal("1269600"):
        errors.append("Joint Tax Payable != 1269600")

    # assert no error message has been registered, else print messages
    assert not errors, "Errors occured:\n{}".format("\n".join(errors))


def test_stdtax2(monkeypatch):
    errors = []

    monkeypatch.setattr('builtins.input', lambda _: "exit")
    next_calc, mpf1, mpf2, jtax, btax = main(
        Decimal("2500000"), Decimal("1800000"))
    ...
    if not round(mpf2, 2) == Decimal("18000"):
        errors.append("Person 2 MPF != 18000")
    if not round(btax, 2) == Decimal("634800"):
        errors.append("Combined Tax Payable != 634800")
    if not round(jtax, 2) == Decimal("639600"):
        errors.append("Joint Tax Payable != 639600")
